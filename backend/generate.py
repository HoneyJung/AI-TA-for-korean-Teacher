...
def fill_template(inference_output):
    ...
    html_template = html_template.replace("♠ 부산광역시 해운대구 우동 1로 104(우동) ☎ 740-7200 ☞ http://www.haedong.es.kr", information["contact"])
    ...
    event_information = "\n".join([f"{item}<br>" for idx, item in enumerate(inference_output["안내사항"], start=1)])
